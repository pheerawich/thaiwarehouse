...
class Welcome extends CI_Controller {
    ...
    function getwarehouse() {
        ...
        $image = $this->input->post('image');
        $status = $this->input->post('status');
        $o = $this->model->test1($nameth, $nameeng, $location, $city, $category, $pricerange, $description, $featured, $contacts, $phone, $email, $image, $status);
    ...
    function update() {
        ...
        $image = $this->input->post('image');
        $status = $this->input->post('status');
        $this->model->update($nameth, $nameeng, $location, $city, $category, $pricerange, $description, $featured, $contacts, $phone, $email, $image, $status);
    ...
    function update1($id) {
        $this->load->view('edit', [
            'r' => $this->model->test3($id)
        ]);
    }

    function delete($id) {
        $this->model->delete($id);
        redirect(base_url('Welcome/NEWSEVENT1'));
    }
    ...
    function saveprofile() {
        ...
        $password = $this->input->post('password');
        $this->model->saveprofile($user, $name, $lastname, $emailr, $password);
    ...
    function search($id = '17') {
        $this->load->view('search', [
            'ser' => $this->model->search($id)
        ]);
    }

    function searchitem($id = '17') {
        $this->load->model('model');
        $this->model->user2_get($id);
        $$this->load->view('search', [
            'ser' => $this->model->search($id)
        ]);
    }
